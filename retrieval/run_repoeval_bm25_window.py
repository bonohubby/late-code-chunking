...
OUTPUT = f"../data/repoeval_custom/repoeval-{os.environ['TARGET'].split('_')[0]}-bm25-window.jsonl"
...
def main():
    ...
    for repo in tqdm(sorted(repos)):
        ...
        for src_path in src_paths:
            ...
            for idx in range(0, len(lines), LINE_SIZE):
                chunk = lines[idx : idx + LINE_SIZE]
                data = {
                    "repo": repo,
                    "file": src_path[len(ROOT_DIR) + len(repo) + 2 :],
                    "chunk": chunk,
                    "tokens": [t for t in DOCSTRING_REGEX_TOKENIZER.findall(" ".join(chunk)) if t],
                }
