...
OUTPUT = f"../data/repoeval_custom/repoeval-{os.environ['TARGET'].split('_')[0]}-unixcoder-lc2.jsonl"
...
def main():
    ...
    error_count = 0
    with torch.no_grad():
        for repo_idx, repo in enumerate(tqdm(sorted(repos))):
            def_kb[repo] = {}
                    ...
                    indices[repo]["data"].append(
                        {
                            ...
                            "cc": tokenizer.decode(ids[idx : idx + CHUNK_SIZE * 2]).split("\n"),
                            "cc_for_ast": chunk.split("\n"),
                        }
                    )
            ...
            print(f"Indexing {repo_idx+1}/{len(repos)}", repo, indices[repo]["index"].ntotal)
        ...
        with open(TARGET) as f, open(OUTPUT, "w") as f_out:
            for line in f:
                ...
                for k, i in enumerate(I[0]):
                    ...
                    for call in get_function_calls(indices[repo]["data"][i]["cc_for_ast"]):
