...
OUTPUT = f"../data/repoeval_custom/repoeval-{os.environ['TARGET'].split('_')[0]}-unixcoder-block.jsonl"
    ...
    with torch.no_grad():
        for repo in tqdm(sorted(repos)):
            ...
            for src_path in src_paths:
                ...
                mini_blocks = [line for line in doc.split("\n\n") if line.strip()]
                blocks = []
                tmp_block = ''
                for mini_block in mini_blocks:
                    tmp_block += mini_block + '\n'
                    if len(tmp_block.split('\n')) > 15:
                        blocks.append(tmp_block.strip().split('\n'))
                        tmp_block = ''
                if not tmp_block:
                    blocks.append(tmp_block.strip().split('\n'))
                ...
                for block in blocks:
                    chunk = block
                    embedding = embed(model, tokenizer, device, code_length, "\n".join(chunk))
        ...
        with open(TARGET) as f, open(OUTPUT, "w") as f_out:
            for line in f:
                ...
                query = " ".join(prompt_lines[-LINE_SIZE:])
